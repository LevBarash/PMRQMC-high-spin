...
int main(int argc, char* argv[]){
	...
	if(mpi_rank == 0) std::cout << "Reading Hamiltonian input text file " << fileName << std::endl;
	vector<pair<complex<double>, vector<int>>> data = data_extract(fileName);
	CDPdata = CDPconvert(data); N = CDPdata.NumOfParticles;
	if(N == 0){
		std::cout << "Error: no particles found. At least one particle must be described by the Hamiltonian." << std::endl;
		MPI_Finalize(); exit(1);
	}
	twoSplus1 = CDPdata.twoSplusOne; Svalue = (twoSplus1-1)*0.5;
	vector<vector<int>> PermMatrixColumn = Convert_perms(CDPdata.Permutations, N);
	Nop = CDPdata.Permutations.size();
	if(mpi_rank == 0) resume_calc = check_QMC_data();
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Bcast(&resume_calc,1,MPI_INT,0,MPI_COMM_WORLD); init_rng();
	if(mpi_rank == 0 && resume_calc == 0){
		vector<vector<int>> cyclesTr = Nullspace_n(PermMatrixColumn, twoSplus1);
		Ncycles = cyclesTr.size() == 0 ? 0 : cyclesTr[0].size();
		for(i=0;i<Ncycles;i++){
			cycles.push_back(vector<int>(Nop));
			for(k=0;k<Nop;k++) cycles[i][k] = cyclesTr[k][i];
		}
		start_time = MPI_Wtime(); int fixed = 0;
		while(MPI_Wtime()-start_time < MAX_FIX_TIME) if(fix_cycles()) { fixed = 1; break;}
#ifndef WORM_UPDATE
	...
	divdiff dd(q+4,500); d=&dd; start_time = MPI_Wtime();
	if(resume_calc){
		load_QMC_data(); init_basic();
	} else{
		MPI_Barrier(MPI_COMM_WORLD);
		MPI_Bcast(&Ncycles,1,MPI_INT,0,MPI_COMM_WORLD);
		int* cycles_arr = new int[Ncycles*Nop];
		if(mpi_rank == 0) for(i=0;i<Ncycles;i++) for(k=0;k<Nop;k++) cycles_arr[i*Nop+k] = cycles[i][k];
		MPI_Bcast(cycles_arr,Ncycles*Nop,MPI_INT,0,MPI_COMM_WORLD);
		if(mpi_rank != 0) for(i=0;i<Ncycles;i++){
			cycles.push_back(vector<int>(Nop));
			for(k=0;k<Nop;k++) cycles[i][k] = cycles_arr[i*Nop+k];
		}
		delete[] cycles_arr;
		init();
	}
