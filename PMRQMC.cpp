...
int main(int argc, char* argv[]){
	...
	string fileName("H.txt"); int i,j,k,o=0;
	std::cout << "Reading Hamiltonian input text file " << fileName << std::endl;
	vector<pair<complex<double>, vector<int>>> data = data_extract(fileName);
	CDPdata = CDPconvert(data); N = CDPdata.NumOfParticles;
	if(N == 0){
		std::cout << "Error: no particles found. At least one particle must be described by the Hamiltonian.\n";
		exit(1);
	}
